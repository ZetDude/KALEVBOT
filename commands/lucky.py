...
def run(message, prefix, alias):
    ...
    try:
        with open(sp + "/important/lucky/lucky{}.txt".format(message.server.id), 'rb') as f:
            luckyDataLocal = pickle.loads(f.read())
    except FileNotFoundError:
        print("local lucky[].txt for this server didn't exist, creating")
        
    try:
        with open(sp + "/important/lucky/luckyGLOBAL.txt", 'rb') as f:
            luckyDataGlobal = pickle.loads(f.read())
    except FileNotFoundError:
        print("luckyGLOBAL.txt for this server didn't exist, creating")
    ...
    allUsersLocal = [x for x in list(message.server.members) if not x.bot]
    ...
    with open(sp + "/important/lucky/lucky{}.txt".format(message.server.id), 'wb') as f:
